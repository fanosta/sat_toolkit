...
cdef class CNF:
    ...
    cdef bytes _to_smt2(self, const char **varnames, int is_bitvec):
        cdef fmt_buf buf = make_buf()
        cdef size_t i, j, start
        cdef int lit

        for i in range(<size_t> self.nvars):
            if is_bitvec:
                buf_printf(&buf, "(declare-fun %s () (_ BitVec 1))\n", varnames[i])
            else:
                buf_printf(&buf, "(declare-fun %s () Bool)\n", varnames[i])

        cdef const char *true_fmt = NULL
        cdef const char *false_fmt = NULL
        ...
        for i in range(self.start_indices.size()):
            ...
            j = 0
            buf_printf(&buf, "(assert (or")
            while 1:
                lit = self.clauses[start + j]
                if lit == 0:
                    break

                if lit > 0:
                    buf_printf(&buf, true_fmt, varnames[lit - 1])
                else:
                    buf_printf(&buf, false_fmt, varnames[(-lit) - 1])

                j += 1
            buf_printf(&buf, "))\n")

        res = <bytes> buf.buf[:buf.len]
        free_buf(&buf)
        return res


    def to_smt2(self, varnames: List[str], sort='bool') -> str:
        cdef ssize_t i
        cdef ssize_t numvars = len(varnames)
        cdef const char **_varnames = NULL

        cdef int is_bool = sort == 'bool', is_bitvec = sort == 'bitvec'

        if not is_bool and not is_bitvec:
            raise ValueError("sort must be either 'bool' or 'bitvec'")
        if numvars != self.nvars:
            raise ValueError("invalid numver of variable names")

        _varnames = <const char **> malloc(sizeof(_varnames[0]) * numvars)
        if _varnames == NULL:
            raise MemoryError()

        try:
            encoded = [None] * numvars
            for i in range(numvars):
                encoded[i] = varnames[i].encode()
                _varnames[i] = encoded[i]

            return self._to_smt2(_varnames, is_bitvec).decode()
